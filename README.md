...
Musl builds ship with Abseil stack tracing disabled (`PDFIUM_USE_MUSL=1`) because musl does not export `execinfo`/`backtrace`; we fall back to a stub so the library still links but without unwinder support. Toolchains are pulled from `https://github.com/rtissera/musl-cc/releases/download/20251208/`—if you need another mirror, override `MUSL_URL` in the workflow/CI environment for your fork. (CI currently runs only the `arm`/`arm64` musl variants; x64/riscv64 musl jobs were disabled until their toolchains become stable.)
